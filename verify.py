...
class Douban():
    ...
    def base64_api(self, img):
        ...
        data = {"username": "usrnm", "password": 'pwd', "typeid": 33, "image": b64}
    ...
    def input_username_password(self):
        ...
        time.sleep(1)
        self.driver.save_screenshot(os.getcwd()+"/reimgs/0.jpg")
        try:
            print('有验证码')
            ...
            time.sleep(0.2)
            self.driver.find_element(By.ID, 'username').send_keys('usrnm')  # 模拟键盘输入
            time.sleep(0.1)
            self.driver.find_element(By.ID, 'password').send_keys('pswd')  # 模拟键盘输入
            time.sleep(0.2)
    ...
    def login(self):
        ...
        time.sleep(0.5)
        self.driver.save_screenshot(os.getcwd()+"/reimgs/douban.jpg")
